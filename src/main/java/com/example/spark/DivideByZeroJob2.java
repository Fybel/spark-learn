...
public class DivideByZeroJob2 {
    public static void main(String[] args) {
        // test job error in the last
        SparkConf conf = new SparkConf().setAppName("DivideByZeroJob2");
        ...
        List<Integer> collect2 = rdd2.map(x -> x / 3).collect();
        ...
        // 会导致 /0 异常
        List<Integer> collect3 = rdd3.map(x -> x / 0).collect();
